      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 w-full">
        <div className="mb-16 text-center gsap-reveal">
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '1.125rem', color: 'var(--color-text-tertiary)' }}>
            Señal abierta — hablemos de seguridad, datos o industria.
          </p>
        </div>
            ...
            <GlassCard glow>
              <div className="text-center">
                ...
                <p className="text-text-tertiary italic mb-6">
                  "Hola Juan Pablo, vi tu perfil y me gustaría hablar sobre..."
                </p>
                <MagneticButton
                  href={`mailto:${EMAIL}`}
          ...
          <div className="gsap-reveal">
            <GlassCard>
              ...
              <div className="space-y-3">
                {Object.entries(LINKS).filter(([, url]) => url && !url.includes('MAYA_')).map(([key, url]) => (
                  <MagneticButton
                    key={key}
                    href={url}
                    target="_blank"
                  ...
                  >
                    <span className="font-medium capitalize">
                      {key === 'linkedin' ? '💼 LinkedIn' : key === 'github' ? '⚡ GitHub' : key === 'instagram' ? '📸 Instagram' : key}
                    </span>
